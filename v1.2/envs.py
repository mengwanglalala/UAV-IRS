...
def phase_from_subset(M,N):
    ...
    return phase_g, phase_H, phase_f
...
def environment(state, action, config):
    ...
    USER = config.USER
    IRS = state_next
    d_AI = math.sqrt((AP[0] - IRS[0]) ** 2 + (AP[1] - IRS[1]) ** 2 + (AP[2] - IRS[2]) ** 2)
    d_IU = math.sqrt((IRS[0] - USER[0]) ** 2 + (IRS[1] - USER[1]) ** 2 + (IRS[2] - USER[2]) ** 2)
    d_AU = math.sqrt((AP[0] - USER[0]) ** 2 + (AP[1] - USER[1]) ** 2 + (AP[2] - USER[2]) ** 2)
    ...
    phase_g, phase_H, phase_f = phase_from_subset(M, N)

    amplify_g = LogdistancePropagation(d_AU, d_ref, L0 + Ld, exponent_AU)
    amplify_H = LogdistancePropagation(d_AI, d_ref, L0 - gain, exponent_AI)
    amplify_f = LogdistancePropagation(d_IU, d_ref, L0, exponent_IU)
    ...
    reward = snr_min * 1e06
    return reward, state_next, done , theta
